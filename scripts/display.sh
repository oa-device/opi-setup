...
if echo "$AVAILABLE_RESOLUTIONS" | grep -q "$PREFERRED_RESOLUTION"; then
    /usr/bin/python3 "${CURRENT_DIR}/gnome-randr.py" --output "$DISPLAY_NAME" --mode "$PREFERRED_RESOLUTION" --rate "$PREFERRED_RATE" --rotate left --scale 2
else
    ...
    if echo "$AVAILABLE_RESOLUTIONS" | grep -q "$ALTERNATE_RESOLUTION"; then
        /usr/bin/python3 "${CURRENT_DIR}/gnome-randr.py" --output "$DISPLAY_NAME" --mode "$ALTERNATE_RESOLUTION" --rate "$PREFERRED_RATE" --rotate left --scale 1
    else
